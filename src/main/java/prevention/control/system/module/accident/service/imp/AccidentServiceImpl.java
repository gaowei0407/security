...
public class AccidentServiceImpl implements AccidentService {
    ...
    public boolean deleteAccidentById(String accidentId) {
        ...
        int accident = accidentMapper.deleteAccidentById(accidentId);

        int accidentReport = accidentMapper.deleteAccidentReportById(accidentId);

        int behavior = accidentMapper.deleteAccidentBehaviorById(accidentId);

        int macroscopic = accidentMapper.deleteAccidentMacroscopicLawOfById(accidentId);

        int preventionAndControl = accidentMapper.deleteAccidentPreventionAndControlById(accidentId);

        int reasons = accidentMapper.deleteAccidentReasonsById(accidentId);

        if (accident!=1||accidentReport!=1||behavior!=1||macroscopic!=1||preventionAndControl!=1||reasons!=1){
            return false;
        }
        return true;
    }
